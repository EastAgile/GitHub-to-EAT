...
- `deps` — each issue's "blocked by" dependencies become blockers on its story,
  ...
  blocker line up to 255 characters, and `--engine direct` keeps fewer of them
  when the title uses accented or non-Latin characters. What the direct engine
  writes is always the start of what the server writes. Tracked as EAT
  #482645. The server engine needs a tracker new enough to accept
